# Personal ChordPro Viewer

A lightweight personal ChordPro viewer and manager for live performance use.

## Demo

https://over-keys.github.io/Personal-ChordPro-Viewer/

## Features

- Add, edit, import, and export songs in ChordPro format
- Search songs by title, artist, and tags
- Auto-scroll with adjustable [S]tart / [E]nd markers
- Built-in YouTube support
- Sticky notes on charts
- Chord formatting and font settings
- JSON backup and restore for songs and notes

## Files

- `index.html` — main application
- `manual.html` — user manual

## Notes

- This app is designed for personal use

## Content and Copyright Notice

- This tool is intended only for data that users are legally permitted to use. Please do not use it to copy, store, share, or publish any data in violation of the terms of external services or applicable copyright law.
